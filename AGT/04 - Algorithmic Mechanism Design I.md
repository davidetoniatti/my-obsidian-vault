...
Informalmente, il mechanism design si applica a *scenari reali e complicati* per via della natura *egoistica* degli agenti che popolano tale situazione: ad esempio il traffico mattutino sul GRA, dato dalle persone che vogliono arrivare comodamente nei luoghi di lavoro senza mezzi pubblici; oppure il continuo utilizzo di combustibili fossili per via del basso costo e facile impiego. Questi sono scenari ordinari, nonostante ciò che vogliono veramente le persone sono le strade libere e lo stop all'uso dei combustibili fossili.

L'idea nel Mechanism design è quella di ottenere delle situazioni ottimale per tali scenari, ovvero strade vuote e uso di fonti rinnovabili, incentivando gli agenti a comportarsi *bene*, in modo socialmente utile. Rispetto agli esempi descritti, tali incentivi possono essere un miglior sistema di trasporto pubblico, oppure degli incentivi per l'utilizzo di fonti rinnovabili.
Dunque alla base del Mechanism Design vi è la progettazione di un sistema che, immerso in un ambiente popolato da agenti egoistici (dunque, si tratta di progettare un gioco), deve raggiungere un certo obiettivo. A tale scopo, il sistema incentiva gli agenti a cooperare col sistema tramite un sistema di pagamento che premia gli agenti che collaborano al raggiungimento dell'obiettivo: deve generare una situazione di equilibrio, in cui agli agenti non conviene fare una scelta diversa rispetto a quella *giusta* per il sistema.

Aspetto fondamentale nella progettazione di meccanismi è che si applica in situazione in cui gli agenti hanno determinate preferenza sull'outcome del gioco: si vuole, date queste preferenze, decidere un outcome che soddisfa tutti gli agenti. Il problema in queste aggregazioni, è che le preferenze effettivamente enunciate dagli agenti al sistema possono non rispettare le reali preferenze, cioè i giocatori possono mentire al sistema per ottenere dei vantaggi. Dunque, idealmente, il sistema deve incentivare i giocatori a esprimere la preferenza reale, cioè a dire la *verità*.
...
Nei problemi di mechanism design ci sono alcune informazioni che conoscono i giocatori che sono **private** che devono essere dichiarate al sistema; potenzialmente un giocatore può dichiarare un informazione diversa per manipolare il sistema.
altro punto di vista, problema di ottimizzazione: calcolare cammino minimo su rete con pesi non noti: gli archi sono gestiti da agenti egoistici per cui solo loro conoscono i pesi degli archi e li devono dichiarare al sistema. Il sistema deve incentivare il giocatore a dire il peso reale di ogni arco, in modo da calcolare un cammino realmente minimo.
