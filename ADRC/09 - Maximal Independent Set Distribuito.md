...
Dato un grafo $G=(V,E)$, un suo **Independent Set** è un sottoinsieme di vertici $M \subseteq V$ tale che non contiene nessuna coppia di nodi adiacenti. Trovare un independent set **massimo** è noto essere un problema _NP-Hard_. Un independent set **massimale** invece è un independent set $M$ il quale non può essere ingrandito ulteriormente con l'aggiunta di un qualsiasi altro nodo in $V\setminus M$.
...
L'insieme $M$ ritornato dall'algoritmo è un independent set massimale. Infatti $M$ è sicuramente un insieme indipendente in quanto ogni volta che si uniscono $S$ e $M$, l'algoritmo rimuove da $V$ tutti i nodi vicini $\Gamma(S)$, dunque qualsiasi altro nodo inserito in futuro non avrà vicini in $M$. Infine $M$ è massimale in quanto ogni nodo in $V$ o viene selezionato per appartenere ad $S$, oppure viene rimosso perché appartiene a $\Gamma(S)$. Perciò, aggiungendo in $M$ un nodo in $V\setminus M$ creerebbe un arco all'interno di $M$.
...
![](adrc_img33.png)
Si osserva che la descrizione informale del task distribuito, come lo pseudocodice di sopra, sono presentati come se fosse presente un coordinatore centralizzato che si occupa di costruire l'independent set in base alle scelte random dei nodi: questo è stato fatto perché boh la lecture di merda le presenta cosi, diciamo per semplificare la trattazione.
#TODO definizione protocollo fatta bene (?)
