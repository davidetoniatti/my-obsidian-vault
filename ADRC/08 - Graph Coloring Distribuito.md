...
\forall(u,v) \in E, C(v) \neq C(u)
$$
In particolare, saranno trattati grafi $\Delta$-regolari, cioè grafi in cui ogni nodo ha grado $\Delta$. Si osserva che ogni grafo $\Delta$-regolare **accetta** una $(\Delta+1)$-colorazione.
# Il problema
...
- $P_{init}$: una colorazione iniziale dei nodi $C_{init}: V \to [a]$ tale che $a > \Delta+1$ e tale che ogni  $v \in V$ conosce il suo colore $C_{init}(v)$; 
- $P_{final}$: una colorazione dei nodi $C_{final}:V\to [\Delta+1]$ tale che ogni nodo $v \in V$ conosca il suo colore finale $C_{final}(v)$;
- $\mathcal{LOCAL}\text{ model restriction}$, cioè le restrizioni definite per tale modello; $R = \text{ standard restrictions}$
...
La **Basic Color Reduction Procedure (BCP)** consente di ridurre la colorazione di $G$ da $a$ colori a $\Delta+1$ colori in $a-(\Delta+1)$ round.  Si osserva esplicitamente che la palette $[\Delta +1]$ è nota localmente a tutti gli agenti. 
...
### Il protocollo
```python
each node v in V parallely applies :
	for each phase k == a,a-1,...,Delta+2:
		send C(v) to N(v)
		receive C(N(v))
		if C(v) == k:
			choose j in {1,...,k-1}-C(N(v))
			C(v) = j
```
### Correttezza
...
- **Fatto 1**: ad ogni round $k$, per ogni nodo $v$ con $C(v)=k$ *esiste sempre* almeno un colore $j$ minore di $k$ e diverso dai colori dei nodi in $N(v)$. Questo è vero perché, per ogni nodo $v$ con $C(v)=k$, ci sono esattamente $\Delta$ vicini, i quali possono avere complessivamente al più $\Delta$ colori distinti, e $k$ è sempre maggiore o uguale di $\Delta+2$. Più formalmente, essendo $|C(N(v))| \leq \Delta$ e $k \geq \Delta +2,$ si ha che $$ |[k-1]\setminus C(N(v))| = |[k-1]|-|C(N(v))| \geq \Delta+1 - \Delta = 1$$
- **Fatto 2**: Al termine di ogni round $k>\Delta+1$, la colorazione $C_k: V \to [k-1]$ è *legale*. Questo è conseguenza del fatto che, per definizione di colorazione legale, i nodi che hanno colore $k$ non possono essere adiacenti, di conseguenza, non possono esistere due vicini che cambieranno colore nello stesso round, portando il sistema ad un'eventuale colorazione non valida. Inoltre, per il fatto 1 il colore $j$, scelto da una palette che non contiene colori dei nodi nel vicinato, esiste sempre.
### Complessità
Per costruzione il protocollo **BCP** impiega $a-(\Delta+1)=O(\Delta)$ round. Nel caso peggiore, la $a$-colorazione iniziale potrebbe essere una biezione in $[n]$ ($a=n$). In tal caso, il protocollo **BCP** converge in $O(n-\Delta)$ rounds su una $(\Delta+1)$-colorazione.
La message complexity in questo caso è $O((n-\Delta)\cdot m)$.
## Parallelizzazione di Khun-Wattenhofer
È possibile ridurre il numero di step per calcolare una $(\Delta+1)$-colorazione di un grafo $\Delta$-regolare tramite il **metodo di Khun-Wattenhofer,** partendo da un' $a-$colorazione con $a \geq 2(\Delta +1)$. In particolare, consente di calcolare tale colorazione in tempo $O\left( \Delta \log\left( \frac{a}{\Delta} \right)\right) .$
### Il protocollo
...
$$
F_{i,i+1}: V_{i} \cup V_{i+1}\to [(i-1)(\Delta+1)+1,(i+1)(\Delta+1)]
$$
Se $k$ dispari, sia  $F_{i,i+1}$ la $2(\Delta+1)$-colorazione indotta da $C_{init}$ su $G(V_i \cup V_{i+1},E)$ come nel caso pari per ogni $i \in \{ 1,3,5,\dots,k-4 \}$, e sia $F_{k-2,k-1,k}$ l'ultima colorazione indotta da $C_{init}$ su $G(V_{k-2} \cup V_{k-1} \cup V_{k},E)$, formalmente
$$
F_{k-2,k-1,k}: V_{k-2} \cup V_{k-1} \cup V_{k} \to [(k-3)(\Delta+1)+1,a]
$$
...
Dopo questo primo round, si osserva che il numero di colori utilizzato da tutte le colorazioni $F_{i,i+1}$ per $i=1,3,\ldots,k-1$ è al più $a/2,$ questo perché in ogni insieme dei vertici $V_i \cup V_{i+1}$ per $i=1,3,\ldots,k$ la size della palette diminuisce almeno da $2(\Delta+1)$ ad $(\Delta+1).$ 
Globalmente, l'$a-$colorazione originale per $G=(V,E)$ viene trasformata mediante **BCP** in parallelo in un' $\lfloor \frac{a}{2} \rfloor$-colorazione per $G$, e può usare quindi la palette $[a/2].$
Inoltre, si osserva esplicitamente che la nuova colorazione di $G$ data dalla composizione delle nuove sotto-colorazioni è legale, essendo che $F_{i,i+1}$ usa colori differenti da $F_{j,j+1}$ per ogni $i\neq j.$
Il protocollo itera questa esecuzione parallela, partizionando $V$ ad ogni round come definito in precedenza, ed applicando **BCP** su ogni sottografo indotto, fino a che non si ottiene una singola $(\Delta+1)$-colorazione per tutto il grafo $G$.

Il running time di ciascuno step di dimezzamento della size della palette è $O(\Delta).$ Dato che ad ogni iterazioni si dimezzano i colori, dopo $t=O\left( \log \left( \frac{a}{\Delta} \right) \right)$ esecuzioni parallele si ottiene una $(\Delta+1)$-colorazione del grafo $G.$
### Correttezza
...
In generale, dopo $t$ applicazioni del protocollo **BCP**, si ottiene una $\left( \frac{a}{2^t} \right)$-colorazione legale per il grafo $G.$ Si vuole fermare l'esecuzione del protocollo al più piccolo round $t$ tale per cui vale
$$
\frac{a}{2^t} \leq 2(\Delta +1) \iff 2^t \geq \frac{a}{2(\Delta+1)}
$$
 Allora, per $t=O\left( \log \left( \frac{a}{\Delta} \right) \right)$, si ottiene una $(\Delta+1)$-colorazione legale.
### Complessità
...
Il protocollo $\text{Rand-}2\Delta$ calcola in maniera distribuita una $2\Delta$ colorazione di un grafo $G$ $\Delta$-regolare.
### Il problema
Formalmente, il problema è dato dalla tripla $P = \langle P_{init}, P_{final}, R \rangle$ dove
- $P_{init}$: una colorazione iniziale dei nodi $C_{init}: V \to [a]$ tale che $a > 2\Delta$ e tale che ogni  $v \in V$ conosce il suo colore $C_{init}(v)$; 
- $P_{final}$: una colorazione dei nodi $C_{final}:V\to [2\Delta]$ tale che ogni nodo $v \in V$ conosca il suo colore finale $C_{final}(v)$;
- $\mathcal{LOCAL}\text{ model},\ R$;
### Il protocollo
...
Siano $u,v$ due nodi vicini e sia per assurdo $C_{final}(v) = C_{final}(u)$. Allora i due nodi $u,v$ non possono aver terminato il protocollo nella stessa fase, in quanto $c_u = c_v$ e $c_v \in T_u, c_{u}\in T_{v}$ e dunque per la linea 8 del protocollo i due nodi non terminerebbero. Allora siano $i,j$ le due fasi in cui i due nodi $u,v$ terminano, e sia w.l.o.g. $i<j$, ovvero $u$ termina prima di $v$. Allora dal round $i$ in poi, il colore definitivo $C_{final}(u) = c_u =c_v$ sarà presente all'interno di $F_v$ (riga 12). Dunque per definizione del protocollo, il colore definitivo $C_{final}(v)$ scelto al tempo $j$ certamente non può appartenere a $F_v$, dunque $C_{final}(u) \neq C_{final}(v)$, contraddicendo l'ipotesi iniziale.
### Complessità
...
Si osserva che $|T_{v} \cup F_{v}|\leq\Delta$ perché ogni vicino di $v$ contribuisce con al più un colore a tale insieme e $G$ è $\Delta$-regolare, cioè $|N(v)|=\Delta$. Di conseguenza, $v$ ha a disposizione *almeno* $\Delta$ colori validi per la colorazione finale nell'insieme $[2\Delta] \setminus (T_{v} \cup F_{v})$. Perciò la probabilità che $v$ scelga un colore valido per la colorazione finale, e dunque terminare, è pari a
$$
...
$$
Allora, sia $\mathcal{B}_{v,j} = v \text{ non termina alla fase } j,$ si osserva che $$v \text{ non termina prima di }j = \bigcap_{k=1}^{j-1} B_{v,k} $$  vale quindi
$$
\mathbf{Pr}\left(\mathcal{B}_{v,j}  | \bigcap_{k=1}^{j-1} B_{v,k}\right) \leqslant \frac{1}{2}
$$
per ogni $j>0$.
...
&= \mathbf{Pr}\left(\mathcal{B}_{v,i}|\bigcap_{j=1}^{i-1} \mathcal{B}_{v,j}\right) \mathbf{Pr}\left(\bigcap_{j=1}^{i-1} \mathcal{B}_{v,j}\right)  \\
&=\mathbf{Pr}\left(\mathcal{B}_{v,i}|\bigcap_{j=1}^{i-1} \mathcal{B}_{v,j}\right)\mathbf{Pr}\left(\mathcal{B}_{v,i-1}|\bigcap_{j=1}^{i-2} \mathcal{B}_{v,j}\right)\mathbf{Pr}\left(\bigcap_{j=1}^{i-2} \mathcal{B}_{v,j}\right) \\
&=\mathbf{Pr}\left(\mathcal{B}_{v,i}|\bigcap_{j=1}^{i-1} \mathcal{B}_{v,j}\right)\mathbf{Pr}\left(\mathcal{B}_{v,i-1}|\bigcap_{j=1}^{i-2} \mathcal{B}_{v,j}\right)\dots \mathbf{Pr}\left(\mathcal{B}_{v,3}|\mathcal{B}_{v,1} \cap \mathcal{B}_{v,2}\right) \mathbf{Pr}\left(\mathcal{B}_{v,2}|\mathcal{B}_{v,1}\right)\mathbf{Pr}\left(\mathcal{B}_{v,1}\right)  \\
...
Il protocollo $\text{Rand-}2\Delta$ permette di calcolare in $O(\log{n})$ passi una $2\Delta$-colorazione di $G$ con alta probabilità. Ma $G$ è $\Delta$-regolare, dunque l'obiettivo è calcolare un $(\Delta+1)$-coloramento, abbattendo la complessità del protocollo **BCP**.
...
1. Ogni nodo $v \in V$ che partecipa ad una fase sceglie *uniformly at random* un bit $c(v) \in\{ 0,1 \}$; se $c(v)=0$, allora $v$ non partecipa alla scelta di un colore e viene direttamente fatto passare alla fase successiva;
...
$$
Quindi la probabilità che $v$ termini nella fase $j$ condizionato al fatto che non ha terminato prima, per ogni $j>0$, è almeno $\frac{1}{4}$.
Allora, sia $\mathcal{B}_{v,j} = v \text{ non termina alla fase } j,$ si osserva che $$v \text{ non termina prima di }j = \bigcap_{k=1}^{j-1} B_{v,k} $$  vale quindi
$$
\mathbf{Pr}\left(\mathcal{B}_{v,j}  | \bigcap_{k=1}^{j-1} B_{v,k}\right) \leqslant \frac{3}{4}
$$
