...
Come introdotto in 1, bisogna descrivere formalmente il *task* specificando:
...
#TODO rifare formalizzazione problema come x gli altri problemi
## Il problema
Dato un grafo $G=(V,E)$, dove $V$ è l'insieme delle entità e $G$ rappresenta la topologia della rete di comunicazione, un messaggio $m \in \{ 0,1 \}^*$ e una funzione di stato
$$
\mathcal{S} :V \to \{ \text{informed}, \text{notInformed} \}
$$
si vuole progettare un protocollo che propaghi $m$ a tutti i nodi.
### Configurazioni del sistema
Le configurazioni del sistema sono tutte le possibili combinazioni degli stati delle entità.
L'insieme delle configurazioni iniziali è composto da tutte le configurazioni nelle quali è presente una singola entità informata, corrispondente al nodo che detiene l'informazione da propagare agli altri nodi. Formalmente, l'insieme delle configurazioni iniziali è composto da configurazioni in cui
$$
\exists! s \in V : \mathcal{S}(s) = \text{informed}
$$
Si osserva che l'insieme delle configurazioni iniziali ha cardinalità pari a $n$.
La configurazione finale invece è unica e corrisponde alla configurazione in cui tutti i nodi ricevono l'informazioni, formalmente è la configurazione in cui
$$
\forall v \in V, \mathcal{S}(v) = \text{informed}
$$
...
if state == "INITIATOR": ## stato
  ...
  receiving(message): ## evento
    send(message) to N(x) ## azione
```
Senza dimostrarlo formalmente per ora, si osserva facilmente che questo protocollo permette al sistema di raggiungere la configurazione finale desiderata, cioè questo protocollo porta a termine il *task* richiesto dal problema broadcast in tempo finito. Ciò nonostante, il protocollo non termina mai, dato che un qualsiasi nodo nello stato `SLEEPING` ogni volta che riceve un messaggio lo inoltra, anche dopo averlo ricevuto la prima volta (si osserva che nella rete, circola solo un messaggio $m$).
...
Il protocollo modificato è il seguente
```python
if state == "INITIATOR":
  spontaneously:
	send(message) to N(x)
...
$$
esiste un tempo finito $t \in \mathbb{R}^+$ tale che ogni nodo di $G$ ha ricevuto il messaggio $m$ *almeno* una volta, ovvero
$$
\forall v \in V \ \mathcal{S}(v) = \text{DONE}
$$
#### Dimostrazione
Sia $L_d$ l'insieme dei nodi distanti esattamente $d$ dalla sorgente $s$. Si dimostra per induzione su $d$ che esiste un tempo $t_d$ tale che tutti i nodi in $L_d$ hanno ricevuto almeno una volta $m$, ovvero che lo stato dei nodi in $L_d$ al tempo $t_d$ sia `DONE`.
Per $d=0$ è banale in quanto $L_0 = \{s\}$. Si considera come caso base $d=1$. Per definizione del protocollo, in un certo istante finito la sorgente invia in maniera spontanea il messaggio a tutti i nodi in $N(s)$ e per definizione di $L_d$, vale $L_1 \equiv N(s)$. Per l'assunzione dell'**affidabilità totale**, non ci saranno fallimento durante la trasmissione, quindi per $d=1$ esiste un istante $t_1 \in \mathbb{R}^+$ entro il quale tutti i nodi in $L_1$ verranno informati.
Sia l'ipotesi vera fino ad un certo $d-1$: si dimostra che l'ipotesi è vera anche per $d$, formalmente
$$
\forall x \in L_{d}, \ \exists t_{d} \in \mathbb{R}^+ : \mathcal{S}(x) = \text{DONE}
$$
Si consideri un qualsiasi nodo $x \in L_d$ che non si trova già nello stato `DONE`. Per definizione, esiste un nodo $y \in L_{d-1}$ tale che $(y,x) \in E$ (questo fatto è vero per le ipotesi di *connessione* e *link bidirezionali*). Se $y$ è la sorgente $s$, allora $x \in L_1$ e l'ipotesi vale per il passo base. Se $y$ non è la sorgente, allora certamente $y$ avrà ricevuto il messaggio mentre era nello stato `SLEEPING`. Per definizione del protocollo $y$ avrà inviato il messaggio al suo vicinato $N(y)$, di cui $x$ fa parte. Per l'**affidabilità totale**, il messaggio arriva in tempo finito e intatto al nodo $x$. Infine per l'ipotesi di **connettività** di $G$
$$
\forall v \in V, \exists k \in \mathbb{N}, k \leqslant diam(G) : v \in L_{k}
$$
### Terminazione
Per definizione, un nodo termina localmente il protocollo quando entra nello stato `DONE`. Si osserva inoltre che un nodo nello stato `DONE` non può transitare in nessun'altro stato. Per la dimostrazione di correttezza, esiste un tempo $t \in \mathbb{R}^+$ nel quale tutti i nodi si trovano nello stato `DONE`, dunque il protocollo termina globalmente all'istante $t$.
### Message complexity
...
In generale, non ha senso calcolare la complessità temporale in un sistema asincrono. Dunque per poter analizzare la complessità temporale del protocollo sono necessarie due assunzioni: **clock sincronizzati** e **bounded delay**.
...
$$
Si riesegua il protocollo con gli stessi ritardi di trasmissione su $G'$.
A livello di stato locale interno, ai due nodi $x,y$ non cambia nulla: le esecuzioni su $G$ e $G'$ sono identiche. Infatti sia $x$ che $y$ in $G'$ vedono un arco uscente laddove in $G$ c'era l'arco $(x,y)$, ma i nodi non sono in grado di distinguere i nodi agli estremi degli archi (le porte per $x$ e $y$ sono le stesse in entrambi gli ambienti).
Allora sapendo che da quella porta né $x$ né $y$ trasmettono messaggi in $G$, certamente nessuno dei due trasmetterà sui relativi archi che li collegano a $z$ in $G'$. Infatti i due ambienti dal punto di vista locale sono **identici** (fact 1), perciò compiono le stesse azioni, ovvero non trasmettere nulla.
Dato che $z$ è collegato solamente a $x$ e $y$, non può ricevere il messaggio da altri nodi, perciò il nodo $z$ non verrà mai informato, dunque il protocollo $P$ non è corretto, assurdo.
## Osservazioni finali
...
In questa sezione verrà studiato il problema del broadcasting su _ipercubi_ e verrà mostrato come l'aggiunta della conoscenza della rete da parte dei nodi, combinata alla simmetria della rete stessa, può portare alla creazione protocolli asintoticamente migliori del _Flooding_.
...
Si osserva che riducendo il grado massimo dei nodi di un grafo, come conseguenza si ottiene la sua sparsificazione.
...
Per dimostrare che il diametro di un ipercubo etichettato $H_d$ ha diametro esattamente $d=\log_2n$ è necessario dimostrare che per ogni coppia di nodi $x,y \in V$ esiste un cammino $P=x \rightsquigarrow y$ tale che la sua lunghezza è minore uguale a $d$.

Siano $x = \langle x_d,x_{d-1},\dots,x_j,\dots,x_1\rangle$ e $y = \langle y_d,y_{d-1},\dots,y_j,\dots,y_1\rangle$ due nodi distinti. Allora scorrendo verso sinistra le due stringhe, si incontrerà un indice $j_1$ tale che $x_{j_1} \neq y_{j_2}$.
Per costruzione dell'ipercubo etichettato, esiste un vicino $x'$ di $x$ tale che differisce nel $j_{1}$-esimo bit, dunque tale che $x'_{j_1} = y_{j_1}$. Si osserva che tale vicino esiste sicuramente, dato che $x'$ e $x$ sono a distanza di Hamming uno. Sia dunque l'arco $(x,x')$ il primo arco del cammino $P$.
A questo punto vale che per ogni $i=1,\dots,j_1$ vale che $x'_i = y_i$. Si considerino ora i bit $j_1,\dots,d$ delle etichette di $x'$ e $y$. Se tra queste etichette non ci sono bit differenti, allora $x'=y$ e dunque il cammino $P$ è concluso.
Altrimenti, sempre scorrendo verso sinistra le due stringhe, sia $j_2$ il primo indice incontrato tale che $x'_{j_2} \neq y_{j_2}$ con $d \geqslant j_2 > j_1$. Come prima, esiste un vicino $x''$ di $x'$ tale che $x''_{j_{2}} = y_{j_{2}}$. Sia quindi $(x',x'')$ il secondo arco del cammino $P$.
Procedendo in questo modo, prima o poi si arrivare ad un nodo $x^* = y$. Nel cammino vengono considerati i nodi a distanza di Hamming 1 l'uno dall'altro, e per ogni arco aggiunti al cammino $P$ ci si avvicina di un salto alla destinazione. Dato che le etichette sono composte da $d$ bit, la distanza tra $x$ e $y$ è al più $d$, dunque il cammino costruito con tale metodo può avere lunghezza al più $d$.
Dato che $x,y$ sono due nodi qualsiasi, il diametro è esattamente pari a $d$.
### Protocollo Hyperflood
Tornando al problema del broadcasting, si progetta il protocollo **Hyperflood**, in grado di risolvere il task sfruttando fortemente il fatto che i nodi sanno di far parte di una rete strutturata come un ipercubo etichettato.
...
#### Correttezza
Per il protocollo **Hyperflood**, un nodo che riceve il messaggio lo inoltrerà solo ai suoi vicini connessi da archi con etichetta minore rispetto all'etichetta dell'arco da cui è stato ricevuto il messaggio.
Dunque per dimostrare che ogni entità riceve il messaggio inviato dalla sorgente, si deve dimostrare che, per ogni nodo $x$, esiste un cammino da $s$ ad $x$ tale che la sequenza degli archi in tale cammino è etichettata in ordine decrescente. A tale scopo, si enuncia e dimostra il seguente lemma
```ad-Lemma
...
```
```ad-Dimostrazione
Siano $x = \langle x_d,x_{d-1},\dots,x_1,x_0\rangle$ e $y = \langle y_d,y_{d-1},\dots,y_1,y_0\rangle$ due nodi distinti. Allora le etichette di $x$ e $y$ differiscono in $t \geqslant 1$ posizioni.
Siano $j_1,j_2\dots,j_t$ le posizioni dei bit in ordine decrescente, ossia $j_i > j_{i+1}$. Siano $v_0,v_1,\dots,v_t$ dei nodi dove $v_1=x$ e dove l'etichetta di $v_i$ differisce dall'etichetta di $v_{i+1}$ solo nella posizione $j_{i+1}$. Allora esiste un arco etichettato con $j_{i+1}$ che connette $v_i$ e $v_{i+1}$ e chiaramente $v_t = y$.
...
```
Data l'esistenza di questi cammini per ogni coppia $s,x$ con $x \in V - \{s\}$, il grafo delle comunicazioni del protocollo è connesso e contiene tutti i nodi in $H_d$. Dunque in tempo finito ogni entità riceve il messaggio.
#### Message complexity
...
![](adrc_img12.png)
Per costruzione dell'ipercubo etichettato, i due cammini iniziano per degli archi etichettati con $l_1,l_2: l_{1} < l_{2}$ <u>differenti</u>. Senza perdita di generalità, si assuma che il cammino di sinistra inizi con l'arco $l_1$ e quello di destra con $l_2$. Sia $x'$ il nodo raggiunto da $l_1$ e $x''$ il nodo raggiunto da $l_2$.
![](adrc_img13.png)
...
##### Dimostrazione
In particolare, si dimostra che per ogni $j\leqslant d$ e per ogni nodo $x$ a distanza di Hamming $j$ dalla sorgente, il nodo $x$ sarà informato al tempo $j$.
