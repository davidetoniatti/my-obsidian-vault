...
$$ \delta_{i} = \sum_{j \in [n]-i} e_{ij} $$ dove $e_{ij}$ è la variabile aleatoria tale che
$$
e_{ij} = \begin{cases}
...
Dunque, la porzione di grafo che è già stata costruita non ha alcuna influenza sul prossimo arco che verrà creato.
...
```
In pratica, i due assi del grafico rappresentano $\ln{x}$ e $\ln{y}$ , e invece di rappresentare $y=f(k)$, si rappresenta $\ln{y}=\ln{f(k)}$. In questo modo, se $f(k) = k^{-c}$, il grafico sarà grosso modo una retta, dato che
$$
\ln{y} = \ln{k^{-c}} \approx -c \ln{k}
$$
# Fenomeno Rich get richer
Uno dei fenomeni più noti nelle reti è il cosiddetto fenomeno **rich get richer**. Su descrive il fenomeno con un esempio.
...
![Pasted image 20231226183650](Pasted%20image%2020231226183650.png)
...
3. Al generico passo $t = i > 2$ vengono creati il nodo $i$ e un arco uscente da $i$
	...
	- Con probabilità $1-p$, viene scelto u.a.r. un nodo $j < i$ e viene creato l'arco $(i,k)$, dove $k$ è l'estremo dell'arco uscente $(i,k)$ dal nodo $i$.
### Verifica power law
...
\begin{align*}
      ...
      &= p \cdot \mathcal{P}\left( \mbox{viene scelto } j \right) + (1-p) \cdot \mathcal{P}\left( \mbox{viene scelto un nodo } h : \delta_{h,j} = 1 \right)\\
      &= \frac{p}{i-1} + (1-p)\cdot\mathcal{P} \Big( \bigcup_{h < i : (h,j) \in E} \mbox{viene scelto } h \Big)\\
      \textbf{(1)}&= \frac{p}{i-1} + (1-p)\cdot\sum_{h < i : (h,j) \in E} \mathcal{P} \left( \mbox{viene scelto } h \right)\\
...
-   sia vero per $i \leq k - 1$, $\sum_{1 \leq j < k} \mathcal{P}( (k,j) \in E )$ per ipotesi induttiva, allora per $k = i$ si ottiene
$$
\begin{align*}
...
Stabilita la legge $\mathcal{P}(D_j(t+1) - D_j(t) = 1) = \frac{p}{t} + \frac{(1-p)}{t}D_j(t)$, si definisce una sua approssimazione. Questo perché generalmente le leggi *discrete* e *probabilistiche* sono troppo complesse da analizzare.
...
Si definisce l'insieme $A_t(k) = \lbrace j \leq t : x_j(t) \geq k \rbrace$, ovvero l'insieme dei nodi che hanno grado entrante **almeno** $k$ (secondo l'approssimazione continua e deterministica).
...
A questo punto, si risolve la disequazione per trovare i valori di $j$ che rispettano la disugualianza
$$
\begin{align*}
...
Da come è definito il modello *rich-get-richer*, è ragionevole pensare che la distribuzione dei gradi dipenda *fortemente* dalla fase iniziale del processo di generazione.
A tale ipotesi vi è a supporto un esperimento, di [Salganik-Dodds-Watts](https://www.princeton.edu/~mjs3/salganik_dodds_watts06_full.pdf) del 2006 che mostra quanto è importante e instabile la fase iniziale.
...
# La lunga coda
La distribuzione della popolarità dei nodi di una rete può avere importanti effetti su attività commerciali. Per esempio si consideri un sito di vendita di libri online.
...
Anche in questo caso si è interessati a questioni di popolarità, però invece di sapere quanti nodi hanno grado $k$ (ovvero sono popolari) si è interessati a sapere se il volume di affare di tanti libri poco popolari è paragonabile a quello di pochi libri molto popolari.

Per affrontare tale questione, si considerino i nodi (libri) in ordine di popolarità <u>non crescente</u>.  
Ovvero, siano i nodi, o libri, $\{ 1, ..., n \}$ disponibili nel magazzino online, si assume che essi sono ordinati in modo tale che la popolarità (o grado entrante) $\delta(i)$ del libro $i$ è <u>non maggiore</u> di quella di $i+1$.
$$
...
Ovvero il numero di nodi con popolarità 100 sarà <u>minore</u> di quelli con popolarità 99, che a sua volta sarà più piccolo di quelli con popolairtà 98, ecc...

A questo punto, si costruisce un grafico dove lungo l'ascisse si elencano tutti i nodi che hanno la stessa popolarità con un solo punto: l'ascissa di tale punto è il numero di nodi che hanno quella popolarità; lungo l'asse delle ordinate invece si elenca la popolarità dei gruppi di nodi: in corrispondenza al punto che indica un gruppo di nodi con popolarità $i$, si indica il grado $i$ di questi nodi.
![[Pasted image 20231227053338.png]]
Si osserva che, scambiando gli assi coordinati, si ottiene il grafico che indica il numero $n$ di nodi che hanno grado entrante $k$, cioè il grafico analizzato in queste lezioni.
Sia $d(k)$ la funzione che esprime il **numero** di nodi di grado entrante $k$. Allora il grafico in figura rappresenta grosso modo la funzione $k=d^{-1}(k)$ (ammesso che $d$ sia invertibile).
È possibile dimostrare che se la funzione $f(\cdot)$ che esprime **la frazione** del numero di nodi nodi (libri) che hanno popolarità (grado entrante) $k$ è una **PowerLaw**, allora anche la funzione $d(\cdot)$ (che invece esprime il numero esatto, e non una frazione) è una **PowerLaw**.
Ovvero
$$n = d(k) \approx k^{-c}$$
per qualche $c > 0$.
...
![[Pasted image 20231227054248.png]]
...
Per questo motivo i venditori online cercano sempre di reindirizzare gli utenti verso i prodotti "in coda", per esempio tramite l'utilizzo del cosidetto **recommendation system** i quali invitano gli utenti a visitare le pagine meno popolari.
