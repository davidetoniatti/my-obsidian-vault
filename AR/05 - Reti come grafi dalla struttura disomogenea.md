...
Fino ad ora si sono studiati grafi che modellano reti a livello statistico; infatti, si è usato pesantemente il calcolo delle probabilità. In questo modo, si sono studiate proprietà che valgono *mediamente* per i nodi, e non si è posto interesse sulle *peculiarità* dei singoli nodi.

Si comincia ora una serie di sezioni in cui si cambia il punto di vista dell'analisi delle reti. Si analizzeranno le posizioni dei singoli nodi all'interno della rete per studiare:
...
Negli anni '60 il sociologo statunitense Mark Granovetter, intervistò un gruppo di individui che avevano recentemente cambiato lavoro.
Il sociologo fece loro una serie di domande volte a capire in che modo erano venuti a conoscenza della possibilità di ottenere l'impiego appena ottenuto.
...
Allora, l'esperimento sembra suggerire che non è tanto la forza del legame che ha aiutato a trovare lavoro, **quanto il tipo di informazione che una relazione è capace di veicolare**.
In effetti gli amici stretti di una persona frequentano, grosso modo, gli stessi posti che frequenta tale persona, dunque egli ha accesso alle stesse fonti di informazioni dei suoi amici stretti.
...
Si consideri il grafo $G=(V,E)$ che modella la rete, gli archi che *aumentano* le informazioni di un nodo sono quelli che lo collegano a regioni della rete inaccessibili ai suoi vicini.
...
D'altra parte, è stato [dimostrato](01%20-%20Introduzione%20e%20grafi%20di%20Erdős-Rényi.md#Componenti%20giganti) che una rete contiene con buona probabilità componenti giganti densamente connesse, dunque la presenza di un bridge è poco probabile.
Per questo motivo, si rilassa la definizione: un arco è un **local bridge** se i suoi estremi non hanno vicini in comune: $(u,v) \in E$ è un **local bridge** se $N(u) \cap N(v) = \emptyset$, dove $N(u)$ è l'insieme dei nodi adiacenti (vicini) al nodo $u$.
...
Dunque se c'è una relazione di amicizia fra $a$ e $b$ e una relazione di amicizia fra $a$ e $c$, ossia gli archi $(a,b),(a,c)$ sono **archi forti**, allora è probabile che prima o poi si creerà anche l'arco $(b,c)$, generando una **chiusura triadica**. Quindi per descrivere queste reti è necessario utilizzare grafi dinamici, che evolvono nel tempo. Alla base della creazione di chiusure triadiche si possono individuare ragioni diverse:
...
Ma di preciso quanto deve essere elevato questo valore per poter stabilire che $C$ è una comunità? Inoltre, il coefficiente di clustering è un buon fattore da considerare per identificare le comunità?
# Comunità
## Cut-communities
...
$$
| \{ (u,v): u \in C \land v \in C \setminus V \}| = \min_{C' \subset V: C' \neq \emptyset} (|\{ (u,v): u \in C' \land v \in C' \setminus V \})
$$
...
$$
| \{ (u,v): u \in C \land v \in C \setminus V \}| = \min_{C' \subset V: s \in C' \land t \in V \setminus C'} (|\{ (u,v): u \in C' \land v \in C' \setminus V \})
$$
...
![|center](05-ar_img01.png)
in questo esempio, comunque si scelga un nodo $x$, l'unico taglio minimo è del tipo $(\lbrace x \rbrace, V \setminus \lbrace x \rbrace)$, di dimensione 7. Infatti, aggiungendo un qualsiasi nodo insieme ad $x$ si ottiene un taglio di dimensione strettamente maggiore di 7. Per esempio
$$
12 = \vert (\lbrace u_0, u_1 \rbrace, V \setminus \lbrace u_0, u_1 \rbrace ) \vert > \vert (\lbrace u_0 \rbrace, V \setminus \lbrace u_0 \rbrace ) \vert = 7
$$
...
$$
\frac{\vert N(u) \cap C \vert}{\vert N(u) \vert} \geqslant \frac{1}{2}
$$
...
### Teorema
Sia $G=(V,E)$ un grafo; se $C \subset V$ è una cut-community per $G$ tale che $\vert C \vert > 1$ allora $C$ è una weak web-community per $G$.
#### Dimostrazione
...
$$
\begin{align}
| (C',V \setminus C') | &=  | (C-\{u\}, (V-C) \cup \{u\}) | \\
&= |  (C,V \setminus C) | - | \{ (u,z): z \in N(u) \setminus C \} | + | \{ (u,z): z \in N(u) \cap C \} | \\
&= |  (C,V \setminus C) | - | N(u) \setminus C  | + | N(u) \cap C | \\
&< |  (C,V \setminus C) |
\end{align}
$$
dunque, $C'$ è un sottoinsieme proprio e non vuoto di $V$ e il numero di archi del taglio indotto da $C'$ minore del numero di archi del taglio indotto da $C$. Questo è assurdo in quanto essendo $C$ per ipotesi una cut-community deve essere vero che $(C, V \setminus C)$ è un taglio minimo di $G.$
### SWCP è NP-Completo
Anche se esiste un algoritmo polinomiale che decide se un grafo è partizionabile in cut-community, non si conosce di alcun algoritmo che **decida** in tempo polinomiale se un grafo è partizionabile in web-communities.
...
- se fosse $y \in V-C$ e $z \in V-C$, allora $$| N(x) \cap C | = 0 < | N(x) \cap (V-C) | = 2$$
- se fosse $y \in C$ e $z \in V-C$, allora $$| N(x) \cap C | = 1 = | N(x) \cap (V-C) | $$
- se fosse $y \in V-C$ e $z \in C$, allora $$| N(x) \cap C | = 1 = | N(x) \cap (V-C) | $$
...
Il problema SWCP è in **NP**. Infatti, un certificato è un sottoinsieme di nodi $C \subset V$ e verificare che $C$ e $V-C$ sono strong web-communities richiede tempo polinomiale in $| G |$.
...
Siano $X = \{x_1,\dots,x_n\}$ insieme di variabili booleane e $f = c_1 \land c_{2} \land \dots \land c_{m}$ una formula booleana in forma normale congiuntiva dove ogni clausola $c_j$ contiene tre letterali. Si costruisce un grafo $G$ nel modo seguente.
...
![|center](Pasted%20image%2020240726160159.png)
Si osserva che se $T$ e $F$ sono nella stessa comunità, allora tutti i nodi in figura sono in quella comunità. Invece, se $T$ e $F$ sono in due comunità distinte, allora anche $x_i$ e $\overline{x_{i}}$ appartengono a due comunità distinte. Infatti, poiché $t_i$ e $f_i$ hanno entrambi grado 2, per il [lemma precedente](#Lemma) deve necessariamente essere vero che $t_i, y_i$ fanno parte della stessa comunità di $T$ e $f_i, z_i$ fanno parte della stessa comunità di $F$.
Poiché $y_i$ ha grado 3, allora almeno 1 tra $x_i$ e $\overline{x_{i}}$ deve appartene alla comunità di $T$. Similmente per $z_i$, poiché ha grado 3 allora almeno uno tra $x_i$ e $\overline{x_{i}}$ deve appartene alla comunità di $F$. Poiché $y_i$ e $z_i$ non sono collegati, allora o $x_i$ appartiene alla comunità di $T$ e $\overline{x_{i}}$ alla comunità di $F,$ oppure viceversa. Infine, collocati i nodi $x_i$ e $\overline{x_{i}}$ nelle rispettive comunità, essi si porteranno appresso i rispettivi vicini senza nome.
...
Si osserva che entrambi i metodi consentono di ottenere delle **partizioni nidificate** (**nested**), infatti nei metodi partitivi si genera una partizione a partire da una più grande (approccio **top-down**), e nei metodi agglomerativi si genera una partizione come composizione di altre più piccole (approccio **bottom-up**).
...
La parte più complessa di questo metodo è il calcolo della betweenness.
## Calcolo della edge-betweenness
...
$$
Complessivamente, si calcola $b_{s}(u,v)$ per ogni $(u,v) \in T(s)$ con il seguente ciclo iterativo

> Per ogni livello $h$ da $d$ fino a $1$, calcola $b_{s}(u,v)$ per ogni $(u,v) \in T(s)$ tale che $v \in L_h$:
> $$
b_{s}(u,v) = \frac{\sigma_{su}}{\sigma_{sv}} + \frac{\sigma_{su}}{\sigma_{sv}} \sum_{(v,x) \in T(s)} b_{s}(v,x)
 $$

Si osserva che si assume $\sigma_{ss} = 1$.
