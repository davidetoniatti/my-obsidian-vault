...
## Sei gradi di separazione: l'esperimento di Milgram
Lo psicologo sociale Stanley Milgram nel 1967 condusse il seguente esperimento.
Milgram scelse un destinatario, cioè una persona alla quale doveva essere recapitata una lettera della quale lo stesso Milgram era il mittente, che risiedeva molto lontano rispetto a lui.
Dopodiché, Milgram ha scelto un insieme di persone, dette iniziatori, e ha consegnato ad ognuno di essi una copia della lettera insieme ad una serie di informazioni sul destinatario: nome, indirizzo, occupazione e informazioni personali.
Dunque, Milgram ha chiesto a tali persone di fare in modo di far giungere la loro copia della lettera al destinatario, senza inviargliela direttamente: ogni iniziatore doveva, con l'obiettivo di far giungere la lettera al destinatario nel minor numero di passi possibile, inviarla a un suo <u>diretto</u> conoscente chiedendogli di ripetere le medesime azioni.
...
Intuitivamente parlando, se si considerano cammini composti dai soli archi aleatori, è poco probabile incontrare due volte uno stesso nodo in brevi distanze, dato che gli archi sono distribuiti uniformemente nel grafo.
Dunque, molto probabilmente, in $h$ passi è possibile raggiungere $k^h$ nodi.
...
Si partiziona poi la rete in "**città**" di $k \times k$ individui, e si considera il fenomeno small-world a livello di città.
...
Questo è vero perché nel modello di Watts-Strogatz l'estremo di un arco random uscente da un nodo è scelto uniformemente a caso fra tutti i nodi: non si tiene conto di quanto sono *vicini* i nodi che congiunge l'arco random.
Detto altrimenti, *gli archi random sono **troppo** casuali*.
...
$$
\mathcal{P}((u,v) \in E) = \frac{1}{Z_{u}} \cdot \frac{1}{d(u,v)^q}
$$
dove
- $d(u,v)$ indica la lunghezza del cammino minimo fra $u$ e $v$ **nella griglia**, ossia il cammino non deve contenere archi random;
- $Z_u$ è un **fattore di normalizzazione** definito come $$Z_u = \sum_{v \in V \setminus \lbrace u \rbrace} \frac{1}{d(u,v)^q}$$ perché vale $$\sum_{v \in V \setminus \lbrace u \rbrace} \mathcal{P}( (u,v) \in E ) = 1$$ Poiché la griglia è *simmetrica e periodica*, il fattore di normalizzazione è uguale per tutti i nodi e verrà indicato con $Z$.
...
Si riconsideri l'esperimento di Milgram, dove un mittente `A` che vive a New York vuole inviare un messaggio ad un destinatario `B` che vive a Garbatella.
Certamente la prima cosa che farebbe `A` sarebbe quella di inviare la lettera ad una persona che vive in Europa. Anche se `A` non ha amici diretti in Europa a cui inviare la lettera, passandola ad altri amici negli USA troverà brevemente qualcuno che ha amici che vivono oltre oceano.
Per semplicità si assuma che `A` invii direttamente la lettera ad un suo amico `C` che vive a Mosca. A sua volta `C` cerca tra i suoi amici qualcuno che abiti almeno in Europa Occidentale. Da `C` la lettera passa a `D` che vive a Parigi.
Per fortuna `D` ha un amico in Italia che però vive a Perugia, e quindi la lettera finisce in Umbira. `D` si ricorda di avere un lontano parente `E` nel Lazio, a cui invia la lettera. Dopodiché `E` invia la lettera ad un suo collega `F` che abita a Roma Centocelle, il quale a sua volta invierà la lettera a un amico `G` che abita a Garbatella.
...
Si formalizza quanto osservato considerando una semplice griglia <u>bidimensionale</u>. Fissato un nodo $u$, si partizionano i nodi rimanenti in gruppi in base alla distanza da $u$: i nodi a distanza da da $u$ compresa fra 2 e 4, poi fra 4 e 8, fra 8 e 16, fra 16 e 32, ..., fra $2^h$ e $2^{h+1}$.
...
\begin{align*}
	\mathcal{P}(\mathcal{E})
	&= \mathcal{P}(\bigcup_{x : 2^h \leq d(u,x) < 2^{h+1}} \mathcal{E}_x) \overbrace{=}^{\text{eventi disgiunti}} \sum_{x : 2^h \leq d(u,x) < 2^{h+1}} \mathcal{P}(\mathcal{E}_x)\\
	&\approx \mid\{\text{nodi a distanza $2^{h+1}$ da $u$}\} -  \{\text{nodi a distanza $2^{h}$ da $u$}\}\mid \cdot \mathcal{P}(\mathcal{E}_x)\\
	&= \alpha 2^{2h} \cdot \beta 2^{-2h}\\
	&= \alpha \cdot \beta 
\end{align*}
$$
per qualche costante $\alpha, \beta$.
Dunque la probabilità che un arco random che parta da $u$ consenta un salto di risoluzione è **indipendente** dalla risoluzione, che sia $2^4$ o $2^{100}$.
...
Formalmente, si considera un grafo $G$ tale che i nodi sono disposti su un anello, ossia $V = [n]$ e l'insieme $E$ prima di inserire archi random è $\{ (i,i+1): 1 \leq i \leq n \} \cup \{ (n,1) \}$.
![|center|500](04-ar_img06.png)
All'insieme $E$ vengono aggiungi gli archi random, dove l'arco $(u,v) \in E$ con probabilità
$$
\mathcal{P}((u,v) \in E) = \frac{1}{Z}\cdot \frac{1}{d(u,v)}
$$
...
$$
\begin{align}
Z &=\sum_{v \in V \setminus \{ u \}} \frac{1}{d(u,v)} = 2 \cdot \sum_{1 \leq h \leq \frac{n}{2}} \frac{1}{h} \\
&\leq 2 \sum_{h=1}^{\infty} \frac{1}{h} = 2\ln{n}
\end{align}
$$
#### Lunghezza media del percorso
Scelti uniformemente a caso due nodi $s$ e $t$ in $G$, si applica l'algoritmo di ricerca decentralizzata per calcolare un percorso da $s$ a $t$. Sia $X$ la variabile aleatoria che denota la lunghezza di tale percorso. Allora si può dimostrare che <u>mediamente</u> tale percorso ha lunghezza pari a
$$ E[X] \in O(\ln^2{n}) $$
...
$$
\frac{d(s,t)}{2^{j+1}} < d(u,t) \leq \frac{d(s,t)}{2^j}
$$
Poiché $d(s,t) \leq \frac{n}{2}$ e ad ogni fase si dimezza la distanza fra il nodo che possiede il messaggio e $t$, allora il numero di fasi è $\leq \log_2{n}$.
Sia $X_j$ la durata della fase $j$, ossia il numero di nodi che entrano in possesso del messaggio durante la fase $j$. Allora
$$
X = \sum_{1 \leq j \leq \log_{2}{n}} X_{j}
$$
dunque, per la linearità del valore atteso
$$
E[X] = \sum_{1 \leq j \leq \log_{2}{n}} E[X_{j}]
$$
Dunque per dimostrare il risultato, è sufficiente dimostrare che per ogni $j$,
$$
E[X_{j}] \in O(\ln{n})
$$
Si dimostra ora questo enunciato.
##### Dimostrazione
Si supponga che il messaggio sia in possesso del nodo $v$ durante la fase $j$: allora vale
$$
\frac{d(s,t)}{2^{j+1}} < d(v,t) \leq \frac{d(s,t)}{2^j}
$$
e la fase $j$ termina sicuramente se esiste un arco $(v,z)$ tale che
$$
d(z,t) \leq \frac{d(v,t)}{2} < \frac{1}{2}\frac{d(s,t)}{2^j} = \frac{d(s,t)}{2^{j+1}}
$$
quindi si ottiene un primo lower bound alla probabilità che la fase $j$ termini
$$
\begin{align}
\mathcal{P}(\text{la fase $j$ temina}) &= \mathcal{P}\left( \exists z \in V : (v,z)\in E \land d(z,t) \leq \frac{d(s,t)}{2^{j+1}} \right) \\
&\geq \mathcal{P}\left( \exists z \in V : (v,z)\in E \land d(z,t) \leq \frac{d(v,t)}{2} \right) \\
\end{align}
$$
Si osserva che il maggiore uguale è dato dal fatto che potrebbe essere $d(v,t)=\frac{d(s,t)}{2}$; in tal caso, esisterebbe un arco $(v,z)$ dell'anello, ossia tale che $d(z,t)=d(v,t)-1 < \frac{d(s,t)}{2^j}$, e tale arco farebbe certamente terminare la fase $j$ e tuttavia sarebbe $d(z,t)>\frac{d(v,t)}{2}$.
Sia $I$ l'insieme di tutti i nodi a distanza da $t$ non più di $\frac{d(v,t)}{2}$, ovvero
$$
I \equiv \left\{ u \in V : d(u,t) \leq \frac{d(v,t)}{2} \right\}
$$
Allora
$$
\begin{align}
\mathcal{P}&\left( \exists z \in V : (v,z) \in E \land d(z,t) \leq \frac{d(v,t)}{2} \right)  \\
&= \mathcal{P}(\exists z \in I : (v,z)\in E) \\
&= \mathcal{P}( \exists z \in I : (v,z) \text{ è un arco dell'annello oppure } (v,z) \text{ è un arco random} \\
&= \sum_{z \in I} \mathcal{P}((v,z) \text{ è un arco dell'annello oppure } (v,z) \text{ è un arco random}) \\
&\geq \sum_{z \in I} \mathcal{P}((v,z) \text{ è un arco random})
\end{align}
$$
Si stima ora la distanza $d(v,z)$.
...
$$
\begin{align}
\mathcal{P}((v,z) \text{ è un arco random}) &= \frac{1}{Z} \frac{1}{d(v,z)} \geq \frac{1}{Z} \frac{2}{3d(v,t)} \\
&\geq \frac{1}{2\ln{n}} \frac{2}{3d(v,t)} \\
&= \frac{1}{3d(v,t)\ln{n}}
\end{align}
$$
Allora vale
$$
\begin{align}
\mathcal{P}\left( \exists z \in V : (v,z) \in E \land d(z,t) \leq \frac{d(v,t)}{2} \right) &\geq \sum_{z \in I} \mathcal{P}((v,z) \text{ è un arco random}) \\
&\geq \sum_{z \in I} \frac{1}{3d(v,t)\ln{n}} \\
&= \frac{1}{3d(v,t)\ln{n}} \cdot \mid I \mid
\end{align}
$$
Resta da valutare $\mid I \mid$, e infine calcolare $E[X_{j}]$.
Sia
$$
\mid I \mid = \left|\left\{ u \in V : d(u,t) \leq \frac{d(v,t)}{2} \right\}\right|
$$
e siano $v_s,v_d$ i due nodi in $I$ a distanza massima da $t$, allora
$$
d(v_{s},t) = d(v_{d},t) = \left\lfloor\frac{d(v,t)}{2}\right\rfloor
$$
siano $v_{s1},v_{d_{1}}$ i due nodi adiacenti a $t$, allora $I$ contiene il nodo $t$ e i $\left\lfloor\frac{d(v,t)}{2}\right\rfloor$ a sinistra e a destra di $v_{s1},v_{d1}$ rispettivamente. Allora
$$
\mid I \mid = 1 + 2\left\lfloor\frac{d(v,t)}{2}\right\rfloor \geq 1 + 2\frac{d(v,t)-1}{2} = d(v,t)
$$
![|center](04-ar_img04.png)
Dunque si conclude che
$$
\mathcal{P}(\text{la fase $j$ temina}) \geq \frac{1}{3d(v,t)\ln{n}} \cdot d(v,t)
$$
...
$$
\mathcal{P}(X_{j}\geq h) \leq \left[ 1 - \frac{1}{3\ln{n}} \right]^h
$$
Per definizione di valore atteso, ricordando che $d(s,t) \leq \frac{n}{2}$, vale
$$
\begin{align}
E[X_{j}] &= 1 \cdot \mathcal{P}(X_{j} = 1) + 2 \cdot \mathcal{P}(X_{j} = 2) + 3 \cdot \mathcal{P}(X_{j} = 3) + \dots + \frac{n}{2} \cdot \mathcal{P}\left( X_{j} = \frac{n}{2} \right) \\
&= \mathcal{P}(X_{j} \geq 1) + 1 \cdot \mathcal{P}(X_{j} = 2) + 2 \cdot \mathcal{P}(X_{j} = 3) + \dots + \left(\frac{n}{2}-1\right)\cdot\mathcal{P}\left( X_{j} = \frac{n}{2} \right) \\
&= \mathcal{P}(X_{j} \geq 1) + \mathcal{P}(X_{j} \geq 2) + 1 \cdot \mathcal{P}(X_{j} = 3) + \dots +\left(\frac{n}{2}-2\right)\cdot\mathcal{P}\left( X_{j} = \frac{n}{2} \right) \\
&= \dots \\
&= \mathcal{P}(X_{j} \geq 1) + \mathcal{P}(X_{j} \geq 2) + \mathcal{P}(X_{j} \geq 3) + \dots +\mathcal{P}\left( X_{j} \geq \frac{n}{2} \right) \\
&= \sum_{1\leq h \leq \frac{n}{2}} \mathcal{P}(X_{j} \geq h) \\
&\leq \sum_{1\leq h \leq \frac{n}{2}} \left[ 1 - \frac{1}{3\ln{n}} \right]^h \\
&\leq \sum_{h \geq 0} \left[ 1 - \frac{1}{3\ln{n}} \right]^h = \frac{1}{1-\left[ 1 - \frac{1}{3\ln{n}} \right]} = 3\ln{n}
\end{align}
$$
Dunque per ogni $j$, vale $E[X_j] \in O(\ln{n})$. Si conclude ora la dimostrazione per $E[X]$: $$ \begin{align} E[X] &= \sum_{1 \leq j \leq \log_{2}{n}} E[X_{j}] = \sum_{1 \leq j \leq \log_{2}{n}} 3\ln{n} = \log_{2}{n} \cdot 3\ln{n} \\
...
$$
\mathcal{P}((u,v) \in E) = \frac{1}{Z} \frac{1}{d(u,v)^0} = \frac{1}{Z} = \frac{1}{n-1}
$$
con $Z = \frac{1}{n-1}$ poiché deve essere
$$
\sum_{v \in V \setminus \{ u \}} \mathcal{P}((u,v) \in E) = \sum_{v \in V \setminus \{ u \}} \frac{1}{Z} = 1
$$
Nel caso $q=d=1$ è "facile" entrare in regioni del grafo contenenti nodi sempre più vicini a $t$ (gli insiemi $I$): più precisamente, la probabilità di *dimezzare* la distanza da $t$ è la stessa sia per nodi molto lontani che molto vicini a $t$.
...
Allora, per ogni $u \not\in R$ vale
$$
\mathcal{P}(\exists u \in R: (u,v) \in E) > \frac{\mid R \mid}{n} = \frac{2\sqrt{ n }}{n} = \frac{2}{\sqrt{ n }}
$$
...
e dunque
$$
E[Y] = \sum_{h \geq 0} \mathcal{P}(Y \geq h) \leq \sum_{h \geq 0}\left( 1 - \frac{2}{\sqrt{ n }} \right)^h = \frac{1}{1-\left( 1 - \frac{2}{\sqrt{ n }} \right)} = \frac{\sqrt{ n }}{2}
$$
...
$$
R_{2}=\left\{  u \in [n]: d(u,t) \leq \frac{\sqrt{ n }}{2}  \right\}
$$
Per ogni $v \in R-R_2$, vale
$$
\mathcal{P}(\exists u \in R_{2}: (v,u) \in E) > \frac{\mid R_{2} \mid}{n} = \frac{\sqrt{ n }}{n} = \frac{1}{\sqrt{ n }}
$$
allora, detta $Y_{2}$ la variabile aleatoria che rappresenta il numero di passi per raggiungere da $s$ un nodo in $R_{2}$, vale
$$
\mathcal{P}(Y_{2} \geq h) < \left( 1 - \frac{1}{\sqrt{ n }} \right)^h
$$
e dunque
$$
E[Y] = \sum_{h \geq 0} \mathcal{P}(Y_{2} \geq h) \leq \sum_{h \geq 0}\left( 1 - \frac{1}{\sqrt{ n }} \right)^h = \frac{1}{1-\left( 1 - \frac{1}{\sqrt{ n }} \right)} = \sqrt{ n }
$$
...
$$\mathbb{E}\left[ X \right] \geq a_q \cdot n^{c_q}$$
```
